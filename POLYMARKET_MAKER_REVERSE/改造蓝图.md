...
## 现状速览（参考）
- 入口：`poly_maker_autorun.py` 调用 `Customize_fliter_blacklist.py` 生成候选，再按配置启动 `POLYMARKET_MAKER/Volatility_arbitrage_run.py` 等脚本。
- 配置：`POLYMARKET_MAKER/config/global_config.json` 决定调度频率；`filter_params.json` 定义筛选/高亮阈值；`strategy_defaults.json` 提供挂单策略模板。
- 筛选：`Customize_fliter_blacklist.py` 负责 Gamma 拉市场、早筛、回补订单簿与高亮判定，目前含 ask 区间等价格口径。

## 改造范围与定位
- **仅修改** `Customize_fliter_blacklist.py` 的筛选与高亮相关逻辑，新增价格反转检测所需的参数、计算与输出。
- 若需要新增默认值/参数暴露，可更新 `POLYMARKET_MAKER/config/filter_params.json` 与 README 说明；其余框架文件保持不动。
...
## 代码改造拆解
- **参数层**：
  - 为 CLI 与配置文件新增 `--rev-p1 / --rev-p2 / --rev-window-hours`（对应 p1、p2、t2）、`--rev-lookback-days`（旧段回溯天数，默认 5），并提供默认值；保持与 `filter_params.json` 的映射。
...
- **输出与集成层**：
  ...
  - 维持 `FilterResult.chosen/highlights` 的结构，确保 `poly_maker_autorun.py` 能无缝读取并启动交易。
  - 若高亮只基于反转结果，需要同步更新提示文本/表头说明。

## 测试与验证计划
- **单元/脚本级自测**：
  - 本地运行 `python Customize_fliter_blacklist.py --stream --hl-max-hours 48`，观察反转标记输出，确认不再依赖 ask 区间。
  - 使用 mock 或小范围市场（可指定 `--market-id` 之类的过滤参数若已有）验证短窗口触发与长窗口确认的分支。
- **集成验证**：
  - 运行 `poly_maker_autorun.py --no-repl --command "list"`，确保筛选结果写入 `POLYMARKET_MAKER/logs/topics_filtered.json`，挂单任务按预期启动。
  - 检查调度频率与 `global_config.json` 一致，且新增参数能在 `filter_params.json` 覆盖。

## 交付物
- 更新后的 `Customize_fliter_blacklist.py`、必要的配置文件以及 README 补充说明全部位于 `POLYMARKET_MAKER_REVERSE` 目录内。
